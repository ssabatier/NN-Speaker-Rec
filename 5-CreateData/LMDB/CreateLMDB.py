...
src_folder_path = '/media/sina/3F8C28A65EBC23F1/Research/Speech/Code/4-CreatePairs/PAIRS/' + choice_feature + '/' + choice_phase
dst_folder_path = '/media/sina/3F8C28A65EBC23F1/Research/Speech/Code/5-CreateData/' + choice_feature + '_' + choice_phase + '_' + 'LMDB'
...
N = num_files
X = np.zeros((N, 2 * FileShape[0], FileShape[1], FileShape[2]), dtype=np.float32) # the number 2 is because the hog features of a pairs should be considered separately
...
for f in glob.glob(os.path.join(src_folder_path, "*.npy")):
    ...
    X[Rand_idx[counter], 0:3, :, :] = numpy_array[:, :, :, 0]
    X[Rand_idx[counter], 3:6, :, :] = numpy_array[:, :, :, 1]
